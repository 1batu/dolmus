...
| ⚖️ Kredi disiplini | Üç banka artık birbirini görüyor. Kredi tek kapıdan (`checkLoan`) geçer: **ödeme gücü** (son 7 günün günlük net kârı + mevcut taksitler; taban ₺5.000) üzerinden **taksit/gelir oranı %55'i** aşamaz, **bankalar arası ortak tavan** var (20× katsayı — tek bankanın 14'ünden yüksek, üçünün toplamından düşük), aynı anda en fazla **4 banka kredisi**. Senet taksitleri de yüke sayılır: senetle araç almak banka kapasitesini daraltır. Banka modalında canlı borç yükü çubuğu, "en çok ₺X çekebilirsin" notu ve ret sebebi yazılı |
...
34. ~~**Google ile giriş + bulut kayıt**~~ ✅ — Firebase Auth (Google, popup) + Firestore. Ayarlar > Hesap'tan giriş; kayıt `saves/{uid}` dokümanına **tek JSON metni** olarak yazılır (Firestore alanlarda `undefined` kabul etmiyor — senetlerin opsiyonel alanları `invalid-argument` veriyordu), yanında özet için `savedAt/day/money`. Yazma 15 sn'de bir kısılır (oyun 2,5 sn'de bir kaydediyor, kota yanmasın). Girişte **çakışma ekranı**: buluttaki ve yereldeki kaydın gün + kasa özeti yan yana, oyuncu seçer — sessizce üzerine yazmak yok. Yapılandırma `VITE_FIREBASE_*` ortam değişkenlerinden gelir; yoksa bulut sessizce kapanır ve oyun yalnız yerel kayıtla tam çalışır
